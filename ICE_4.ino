...
#define wifi_ssid "Samsung Galaxy S9_1914"   //You have seen this before
#define wifi_password "thga9095" //
...
void loop() {
  ...
  if (millis() - timerOne > 10000) {
    ...
    char legoBatmanIronyLevel[7] = "FFFFFF";
    
    sprintf(message, "{\"I want to paint it\":\"%s%\"}", legoBatmanIronyLevel); // %d is used for an int
  ...
  if (millis() - timerTwo > 15000) {
    ...
    char str_temp[3]; //a temp array of size 6 to hold "XX" + the terminating character
    char str_humd[3]; //a temp array of size 6 to hold "XX" + the terminating character
    ...
    sprintf(message, "{\"Max people\":\"%s\", \"People here\":\"%s\"}", classSize, classAttend);
    mqtt.publish("itsForMeeee/testing", message);
    timerTwo = millis();
  }
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (strcmp(topic, "itsForMeeee/testing") == 0) {
